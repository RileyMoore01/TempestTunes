...
function requestAuth(){
    ...
    url += "&scope=playlist-modify-private user-read-private user-read-email user-modify-playback-state user-read-playback-position user-library-read streaming user-read-playback-state user-read-recently-played playlist-read-private";
...
function translateToURIs(tracks){
    ...
    callApi(
    "POST",
    localStorage.getItem("newPlaylistId") + 
    `/tracks?uris=${URIstring}`,
    null,
...
function addTracksToNewPlaylist(){
    if ( this.status == 201 ){
        var data = JSON.parse(this.responseText);
        console.log(data)
        localStorage.removeItem("newPlaylistId")
    }
    else if ( this.status == 401 ){
        refreshAccessToken()
    }
...
function handleCreatePlaylistReponse(){
    if ( this.status == 201 ){
        ...
        callApi("GET",WINTER,null,handleGetTracksResponse)
